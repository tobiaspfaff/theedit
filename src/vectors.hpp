...
#define static_assert_v(cond) static_assertion_failure<cond>();
...
public:
    ...
    explicit Vec (T x, T y) {static_assert_v(n==2); c[0] = x; c[1] = y;}
    explicit Vec (T x, T y, T z) {static_assert_v(n==3); c[0] = x; c[1] = y; c[2] = z;}
    explicit Vec (T x, T y, T z, T w) {static_assert_v(n==4); c[0] = x; c[1] = y; c[2] = z; c[3] = w;}
    explicit Vec (const Vec<2,T>& o, T z) {static_assert_v(n==3); c[0] = o[0]; c[1] = o[1]; c[2] = z;}
    explicit Vec (const Vec<3,T>& o, T w) {static_assert_v(n==4); c[0] = o[0]; c[1] = o[1]; c[2] = o[2]; c[3] = w;}
    explicit Vec (const Vec<3,T>& o, const Vec<2,T>& a) {static_assert_v(n==5); c[0] = o[0]; c[1] = o[1]; c[2] = o[2]; c[3] = a[0]; c[4] = a[1];}
    T &operator[] (int i) {return c[i];}
    const T &operator[] (int i) const {return c[i];}
    inline void serialize(Serializer& s, const std::string& name);
};

template<int n,typename T> void serialize_vec(Serializer& s, Vec<n,T>& v, const std::string& name);
template<int n,typename T> inline void Vec<n,T>::serialize(Serializer& s, const std::string& name) { serialize_vec(s, *this, name); }
...
tpl VecnT abs (const VecnT & u) {VecnT x; for (int i = 0; i < n; i++) x[i] = std::abs(u[i]);}
...
tpl VecnT eldiv(const VecnT& a, const VecnT& b) { VecnT c; for (int i=0; i<n; i++) c[i] = a[i]/b[i]; return c; }
template<int n> Vec<n> toVec(const Vec<n,int>& v) { Vec<n> a; for (int i=0; i<n; i++) a[i] = v[i]; return a; }
template<int n> Vec<n, int> toVeci(const Vec<n>& v) { Vec<n, int> a; for (int i=0; i<n; i++) a[i] = (int)floor(v[i]); return a; }
...
public:
    ...
    explicit Mat (VecmT x, VecmT y) {static_assert_v(n==2); c[0] = x; c[1] = y;}
    explicit Mat (VecmT x, VecmT y, VecmT z) {static_assert_v(n==3); c[0] = x; c[1] = y; c[2] = z;}
    explicit Mat (VecmT x, VecmT y, VecmT z, VecmT w) {static_assert_v(n==4); c[0] = x; c[1] = y; c[2] = z; c[3] = w;}
...
//aa:    static Mat rows (VecnT x, VecnT y, VecnT z) {return Mat<n,3,T>(x,y,z).t();}
    ...
    void serialize(Serializer& s, const std::string& name);
};
template<int m, int n,typename T> void serialize_mat(Serializer& s, Mat<m,n,T>& v, const std::string& name);
template<int m, int n,typename T> inline void Mat<m,n,T>::serialize(Serializer& s, const std::string& name) { serialize_mat(s, *this, name); }
...
tpl MatmnT &operator*= (MatmnT &A, const MatnnT &B) {return A = A*B;}
